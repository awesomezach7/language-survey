...
")
      ...
      (let [direction (string.sub line 6)
        ...
        (if newroom
            (do
              (each [i character (ipairs player.room.players)]
                (when (= character player.name)
                  (table.remove player.room.players i)))
              (set player.room
                   (assert (. rooms newroom) (.. "room not found " newroom)))
      ...
      (let [item (string.sub line 5)]
        (case (lume.find player.room.items item)
         ...
         nil (player.client:send "That item does not exist in this room.  Try using look again.  Perhaps another player took it?\n")))
      (string.find line "^drop ")
      (let [item (string.sub line 6)]
        (var dropped false)
        (each [i name (ipairs player.inventory)]
          (when (and (= name item) (not dropped))
            (set dropped true)
